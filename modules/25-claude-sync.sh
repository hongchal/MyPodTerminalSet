...
PERSIST="${PERSIST_ROOT}/.claude"
LOCAL="${HOME}/.claude"
STAMP="$(date +%Y%m%d-%H%M%S)"

log()  { printf '\033[1;36m[claude-sync]\033[0m %s\n' "$*"; }
warn() { printf '\033[1;33m[claude-sync]\033[0m %s\n' "$*"; }
die()  { printf '\033[1;31m[claude-sync error]\033[0m %s\n' "$*" >&2; exit 1; }

if [[ ! -d "${PERSIST_ROOT}" ]]; then
  warn "${PERSIST_ROOT} 없음 — /DATA1 마운트 / 노드 핀 확인. claude-sync 건너뜀."
  exit 0
fi

# 1. 이미 올바른 symlink면 종료
if [[ -L "${LOCAL}" ]]; then
