...
`/DATA1/hongcheol`이 없는 머신에선 자동으로 `~/MyTerminalSet`으로 fallback.
...
| Env | 기본값 | 용도 |
|-----|--------|------|
| `MTS_REPO` | `https://github.com/hongchal/MyTerminalSet.git` | 다른 fork 쓸 때 |
| `MTS_BRANCH` | `master` | 실험 브랜치 |
| `MTS_DEST` | `/DATA1/hongcheol/dotfiles` (없으면 `~/MyTerminalSet`) | clone 위치 |
...
git clone https://github.com/hongchal/MyTerminalSet.git /DATA1/hongcheol/dotfiles
...
| `30-cli-tools.sh` | tmux, ripgrep, bat, fd, htop, ncdu, tree, jq, gh, lazygit, delta, direnv |
